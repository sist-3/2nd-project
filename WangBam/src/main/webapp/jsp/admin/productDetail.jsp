<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		<%@include file="/jsp/common/adminHeader.jsp" %>
			<div class="admin">
				<h1>상품 상세 정보</h1>
				<form class="form-group" action="admin?type=productUpdate" method="post" enctype="multipart/form-data"
					name="updateForm">
					...
					<div>
						<label for="pd_name">상품명</label> <input type="text" id="pd_name" placeholder="상품명"
							name="pd_name" value="${pvo.pd_name}" data-label="상품명">
					</div>
					<div>
						...
						<div class="address-group">
							<select id="ct_idx" name="ct_idx" data-label="카테고리">
								<c:forEach var="ct" items="${ctvo}">
									<option value="${ct.ct_idx}" <c:if test="${ct.ct_idx eq pvo.cvo.ct_idx}">selected
										</c:if>>${ct.ct_name}</option>
								</c:forEach>
							</select>
					...
					<div>
						<label for="pd_cnt">수량</label> <input type="text" id="pd_cnt" name="pd_cnt" placeholder="수량"
							data-label="수량" value="${pvo.pd_cnt}">
					</div>
					<div>
					...
					<div>
						<label for="price">가격</label> <input type="text" id="pd_price" placeholder="가격" name="pd_price"
							data-label="가격" value="${pvo.pd_price}">
					</div>
					<div>
						<label for="productImg">썸네일이미지</label>
						<div>
							<label for="updateImg" class="file-select">파일 선택 +</label> <input type="file" id="updateImg"
								name="updateImg" onchange="selectImg(this)">
					...
					<div>
						...
						<div>
							<label for="updateDetailImg" class="file-select">파일 선택 +</label> <input type="file"
								id="updateDetailImg" name="updateDetailImg" onchange="selectImg(this)">
							<p>${pvo.pd_detail_img }</p>
						</div>
					</div>
					<input type="hidden" name="pd_idx" value="${pvo.pd_idx}"> <input type="hidden" name="pd_status"
						value="${pvo.pd_status}">
					<div class="buttons">
				...
				<table class="table" id="categoryList">
					...
					<tbody>
						<c:forEach var="cvo" items="${ctvo}" varStatus="st">
							<tr>
								<td class="split-box">
									<input type="text" value="${cvo.ct_name}" name="ct_name${cvo.ct_idx}"
										id="ct_name${cvo.ct_idx}">
									<button type="button" class="admin-btn cancel"
										onclick="updateCategory('${cvo.ct_idx}')">수정</button>
								</td>
								<td>
									<button type="button" class="admin-btn cancel"
										onclick="delCategory('${cvo.ct_idx}')">삭제</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
